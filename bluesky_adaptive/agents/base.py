...
from ..server import register_method, register_variable
...
class Agent(ABC, metaclass=BackwardCompatMeta):
    ...
    def _register_method(self, name: str, method_name: str | None = None, **kwargs):
        """Wrapper to register generic method to bluesky-adaptive server instead of attribute or variable.
        Without kwargs, this will attempt to infer the input schema and output typing from the method signature.
        To call the method, POST to the endpoint
        ``/agent/{agent_name}/{method_name}`` with a JSON body containing the keyword arguments
        to pass to the method. The method should be defined in the agent class.

        Parameters
        ----------
        name : str
            Name by which the variable is accessible through the REST API.
            The name does not need to match the actual name of the variable used in the code.
            The name should be selected so that it could be conveniently used in the API.
        method_name : Optional[str]
            The name of the method, by default the same name used in the REST API.
        """
        register_method(name, self, method_name=method_name if method_name is not None else name, **kwargs)
